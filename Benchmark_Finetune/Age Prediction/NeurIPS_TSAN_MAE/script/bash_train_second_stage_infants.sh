...
lbd=10
beta=1
first_stage_net=./infants_wenyi/ScaleDense/ScaleDense_best_model.pth.tar
save_path=./infants_wenyi/second_stage_test/
label=./data_infants/dataset.xls

train_data=./data_infants/train
valid_data=./data_infants/valid
test_data=./data_infants/test
sorter_path=./TSAN/Sodeep_pretrain_weight/best_lstmla_slen_${batch_size}.pth.tar

# ------ train and set the parameter
CUDA_VISIBLE_DEVICES=1     python  ./TSAN/train_second_stage.py  \
