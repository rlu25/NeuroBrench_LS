...
def main():
    ...
    opt_excel_path = '/opt/localdata/data/usr-envs/wenyi/TSAN/TSAN-brain-age-estimation/test_dataset.xls'
    opt_test_folder = '/opt/localdata/data/usr-envs/wenyi/TSAN/TSAN-brain-age-estimation/test2'
    opt_batch_size = 4
    opt_model = 'ScaleDense'
    opt_use_gender = True
    opt_output_dir = '/opt/localdata/data/usr-envs/wenyi/TSAN/TSAN-brain-age-estimation/pretrained_model/ScaleDense/'
    opt_model_name = f'{opt_model}_best_model.pth.tar'
    opt_num_workers = 8
    opt_npz_name = 'brain_age.npz'
    opt_plot_name = 'test.png'

    
    test_data = Integer_Multiple_Batch_Size(IMG_Folder(opt_excel_path, opt_test_folder),opt_batch_size)
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

    # ========  build and set model  ======== #  
    if opt_model == 'ScaleDense':
        model = ScaleDense.ScaleDense(8, 5, opt_use_gender)
    else:
    ...
    model.load_state_dict(torch.load(os.path.join(opt_output_dir+opt_model_name))['state_dict'])
    ...
    test( valid_loader=test_loader
        ...
        , opt_model=opt_model
        , save_npy=True
        , npy_name=opt_npz_name
        , figure=False
        , figure_name=opt_plot_name)

def test(valid_loader, model, criterion, device, opt_model
    ...
    with torch.no_grad():
        ...
        if save_npy:
            savepath = os.path.join(opt_output_dir,npy_name)
            np.savez(savepath 
                    ,target=target_numpy
        ...
        if figure is True:
            ...
            # plt.show()
            plt.savefig(opt_output_dir+figure_name)
