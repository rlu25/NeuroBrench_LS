...
def read_table(path):
    return(pd.read_excel(path, header=None).values) # default to first sheet
...
class IMG_Folder(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not found:
            raise ValueError(f"No matching subject found for {sub_fn}")
