...
parser.add_argument('--output_dir'     ,default='./trained_wenyi/ScaleDense/',type=str, help="Output dictionary, whici will contains training log and model checkpoint")
...
parser.add_argument('--mae_pretrain_path'      ,default='mae_model.pth'              ,type=str, help="After inference the trained model in test set, a scatter plot will be saved in assigned path. So the plot name need to be appointed. ")
...
parser.add_argument('--epochs'      ,default=150         ,type=int,   help="Total training epochs")
...
parser.add_argument('--weight_decay',default=1e-6        ,type=float, help="L2 weight decay ")
parser.add_argument('--use_gender'  ,default=False        ,type=bool,  help="If use sex label during training")
...
parser.add_argument('--sorter',     default='./TSAN/Sodeep_pretrain_weight/Tied_rank_best_lstmla_slen_8.pth.tar', type=str,   help="When use ranking, the pretrained SoDeep sorter network weight need to be appointed")
