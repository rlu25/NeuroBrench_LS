...
class second_stage_scaledense(nn.Module):
    def __init__(self,nb_filter,nb_block, use_gender=False):
        '''
    ...
    def forward(self,x, dis_age_input): # male_input
        x = self.pre(x)
        x = self.block(x)
        x = self.gap(x)
        x = torch.reshape(x,(x.size(0),-1))
        ...
        # residual_age = x
        # output_age = residual_age + dis_age_input
        return x, x + dis_age_input
