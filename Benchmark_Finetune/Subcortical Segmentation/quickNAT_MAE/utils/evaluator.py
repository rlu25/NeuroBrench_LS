...
def center_pad_or_crop(img, target_H, target_W):
    B, C, H, W = img.shape

    # Crop if too large
    if H > target_H:
        h_start = (H - target_H) // 2
        img = img[:, :, h_start:h_start + target_H, :]
    if W > target_W:
        w_start = (W - target_W) // 2
        img = img[:, :, :, w_start:w_start + target_W]

    # Pad if too small
    pad_H = max(target_H - img.shape[2], 0)
    pad_W = max(target_W - img.shape[3], 0)
...
def restore_to_original(pred, orig_H, orig_W):
    """
    Restore a [B, C, H, W] prediction tensor (e.g., 256×256) back to original size (orig_H, orig_W),
    by either padding or cropping in the center.
    """
    B, C, H, W = pred.shape
    ...
    if H > orig_H:
        h_start = (H - orig_H) // 2
        pred = pred[:, :, h_start:h_start+orig_H, :]
    if W > orig_W:
        w_start = (W - orig_W) // 2
        pred = pred[:, :, :, w_start:w_start+orig_W]

    # Then pad if needed
    pad_H = max(orig_H - pred.shape[2], 0)
    pad_W = max(orig_W - pred.shape[3], 0)
    ...
    pred = F.pad(pred, (pad_left, pad_right, pad_top, pad_bottom))
...
def _segment_vol(file_path, model, orientation, batch_size, cuda_available, device):
    ...
    orig_H, orig_W = volume.shape[2], volume.shape[3]
    ...
    for i in range(0, len(volume), batch_size):
        ...
        batch_x = center_pad_or_crop(batch_x, target_H, target_W)
        if cuda_available and (type(device)==int):
            batch_x = batch_x.cuda(device)
        out = model(batch_x)
        out = restore_to_original(out, orig_H, orig_W)
        # _, batch_output = torch.max(out, dim=1)
...
def evaluate(coronal_model_path, volumes_txt_file, data_dir, device, prediction_path, batch_size, orientation,
    ...
    with torch.no_grad():
        ...
        cvs_dict_list = []
        iou_dict_list = []
        for folder_name, file_name, file_path in file_paths:
            try:
                if need_unc == "True":
                ...
                log.info("Processed: " + file_path + " out of " + str(
                    len(file_paths)))
                save_file = os.path.join(prediction_path, folder_name, file_name)
                ...
                per_volume_dict = compute_volume(volume_prediction, label_names, file_name)
                volume_dict_list.append(per_volume_dict)
