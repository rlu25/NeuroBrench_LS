...
class Solver(object):
        ...
        for epoch in range(self.start_epoch, self.num_epochs + 1):
            ...
            for phase in ['train', 'val']:
                ...
                for i_batch, sample_batched in enumerate(dataloaders[phase]):
                    ...
                    if i_batch < 1 and phase == 'val':  # Only save the first batch of validation
                        self.save_prediction_and_label(output, y, save_dir="/opt/localdata/data/usr-envs/ruiying/Code/NeurIPS/QuickNet/quickNAT_MAE/image_plot", prefix=f"epoch{epoch}")
    ...
    def save_prediction_and_label(self, output, target, save_dir, prefix="sample", affine=None):
    
        os.makedirs(save_dir, exist_ok=True)
        affine = affine if affine is not None else np.eye(4)

        # Take argmax along channel axis (B, C, H, W) -> (B, H, W)
        preds = torch.argmax(output, dim=1).cpu().numpy()
        targets = target.cpu().numpy()

        for i in range(preds.shape[0]):
            pred_nifti = nib.Nifti1Image(preds[i].astype(np.uint8), affine)
            label_nifti = nib.Nifti1Image(targets[i].astype(np.uint8), affine)

            nib.save(pred_nifti, os.path.join(save_dir, f"{prefix}_pred_{i}.nii.gz"))
            nib.save(label_nifti, os.path.join(save_dir, f"{prefix}_label_{i}.nii.gz"))

        print(f"Saved predictions and labels to {save_dir}")
