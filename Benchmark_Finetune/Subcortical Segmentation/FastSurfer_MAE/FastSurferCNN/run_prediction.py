...
import sys
sys.path.insert(0, "/opt/localdata/data/usr-envs/ruiying/Code/NeurIPS/FastSurfer/FastSurfer_MAE")
...
CHECKPOINT_PATHS_FILE = "" #checkpoint files
...
class RunModelOnData:
    ...
    def conform_and_save_orig(self, subject: SubjectDirectory) -> tuple[nib.Nifti1Image, np.ndarray]:
        orig_img = nib.load(subject.orig_name)
        orig_data = orig_img.get_fdata()

        if orig_data.ndim == 4 and orig_data.shape[-1] == 1:
            orig_data = np.squeeze(orig_data, axis=-1)

        if subject.can_resolve_attribute("conf_name"):
            self.save_img(subject.conf_name, orig_data, orig_img, dtype=orig_data.dtype)
        else:
            raise RuntimeError("Cannot resolve output conf_name.")

        return orig_img, orig_data
    ...
    def preprocess_by_plane(self, data: np.ndarray, plane: str, zoom_orig: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        from FastSurferCNN.data_loader.data_utils import transform_axial, transform_sagittal

        # -------- Orientation Transform --------
        if plane == "sagittal":
            data = transform_sagittal(data)
            zoom = zoom_orig[::-1][:2]
        elif plane == "axial":
            data = transform_axial(data)
            zoom = zoom_orig[1:]
        else:  # coronal
            zoom = zoom_orig[:2]
        ...
        # -------- Normalize to [0, 255] --------
        def normalize(img, percentile=99.5):
            img = (img - np.min(img)) / (np.percentile(img, percentile) - np.min(img))
            return np.clip(img, 0, 1) * 255

        data = normalize(data).astype(np.uint8)

        # -------- Center crop/pad 2D slices to 256×256 --------
        def center_pad_crop_2d(img3d, target_shape=(320, 320)):
            H, W = target_shape
            padded = np.zeros((H, W, img3d.shape[2]), dtype=img3d.dtype)
            for i in range(img3d.shape[2]):
                slice_ = img3d[:, :, i]
                h, w = slice_.shape
                if h <= H:
                    h_start = (H - h) // 2
                    h_slice = slice(h_start, h_start + h)
                    src_h = slice(0, h)
                else:
                    src_h = slice((h - H) // 2, (h - H) // 2 + H)
                    h_slice = slice(0, H)

                if w <= W:
                    w_start = (W - w) // 2
                    w_slice = slice(w_start, w_start + w)
                    src_w = slice(0, w)
                else:
                    src_w = slice((w - W) // 2, (w - W) // 2 + W)
                    w_slice = slice(0, W)

                padded[h_slice, w_slice, i] = slice_[src_h, src_w]
            return padded

        data = center_pad_crop_2d(data)
        return data, zoom
    ...
    def get_prediction(
        self, image_name: str, orig_data: np.ndarray, orig_zoom: np.ndarray | Sequence[int],
    ) -> np.ndarray:
        """
        ...
        for plane, model in self.models.items():
            ...
            plane_data, zoom = self.preprocess_by_plane(orig_data.copy(), plane, orig_zoom)

            self.set_model(plane)
            # pred_prob is updated inplace to conserve memory
            pred_prob = model.run(pred_prob, image_name, plane_data, zoom, out=pred_prob)
            # if plane == "coronal":
            #       break
    ...
    def reverse_preprocessing(self, pred: np.ndarray, orig_shape: tuple, plane: str) -> np.ndarray:
        # Remove center padding
        H_orig, W_orig, D = orig_shape
        H_pad, W_pad, _ = pred.shape

        # Compute crop indices
        h_start = (H_pad - H_orig) // 2 if H_pad > H_orig else 0
        w_start = (W_pad - W_orig) // 2 if W_pad > W_orig else 0
        pred = pred[h_start:h_start + H_orig, w_start:w_start + W_orig, :]

        # Reverse transform
        if plane == "sagittal":
            pred = np.transpose(pred, (2, 0, 1))  # z, y, x
            pred = np.flip(pred, axis=2)  # undo sagittal flip
        elif plane == "axial":
            pred = np.transpose(pred, (1, 2, 0))  # y, x, z
            pred = np.flip(pred, axis=1)  # undo axial flip
        else:
            pred = np.transpose(pred, (1, 0, 2))  # y, x, z
...
def main(
        *,
        ...
        remove_suffix: str = "",
        brainmask_name: str = "mri/nii.gz",
        aseg_name: str = "mri/aseg.auto_noCC.nii.gz",
        vox_size: VoxSizeOption = "min",
        device: str = "auto",
        viewagg_device: str = "auto",
        batch_size: int = 1,
        ...
        try:
            ...
            if store_aseg:
                ...
                try:
                    ...
                    LOGGER.warning("No connected components found in aseg; skipping white matter flipping.")
