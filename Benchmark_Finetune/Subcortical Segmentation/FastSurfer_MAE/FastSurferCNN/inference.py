...
class Inference:
    ...
    def center_crop_2d(self, pred, th, tw):
        _, _, H, W = pred.shape
        

        if H > th:
            h_start = (H - th) // 2
            pred = pred[:, :, h_start:h_start + th, :]
        if W > tw:
            w_start = (W - tw) // 2
            pred = pred[:, :, :, w_start:w_start + tw]

        # Pad if smaller
        pad_H = max(th - pred.shape[2], 0)
        pad_W = max(tw - pred.shape[3], 0)
    ...
    def eval(
        ...
        *,
        ...
        # Set correct stacking dim based on plane
        plane_dim_map = {
            "axial": 1,     # stack along Z
            "coronal": 2,   # stack along Y
            "sagittal": 0,  # stack along X
        }
        stack_dim = plane_dim_map[plane]  # plane comes from self.cfg.DATA.PLANE
        ...
        if plane == "sagittal":
            ...
            target_H, target_W = target_shape[2], target_shape[0]
        else:
        ...
        with logging_redirect_tqdm():
            try:
                for batch_idx, batch in tqdm(
                    ...
                    pred = self.center_crop_2d(pred, orig_h, orig_w)
        ...
        #out = torch.cat(all_pred, dim=stack_dim)
