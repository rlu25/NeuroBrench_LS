...
def load_image(
    ...
    **kwargs :
    ...
    #data = np.asarray(img.get_fdata(), dtype=np.uint16)
...
def filter_blank_slices_thick(img_vol: np.ndarray, label_vol: np.ndarray, threshold=0.99) -> tuple[np.ndarray, np.ndarray]:
    """
    Remove slices where >99% of label is background.

    Parameters
    ----------
    img_vol : np.ndarray
        Input image volume of shape [H, W, D] or [H, W, D, C]
    label_vol : np.ndarray
        Label volume of shape [H, W, D]
    axis : int
        Axis along which to slice (default=2 for axial)

    Returns
    -------
    img_slices : np.ndarray
        Filtered image slices
    label_slices : np.ndarray
        Filtered label slices
    """
    select_slices = []
    for i in range(img_vol.shape[2]):
        slice_img = img_vol[:, :, i, 0] if img_vol.ndim == 4 else img_vol[:, :, i]
        zero_ratio = np.sum(slice_img == 0) / slice_img.size
        if zero_ratio < threshold:
            select_slices.append(i)

    select_slices = np.array(select_slices)
    img_vol = img_vol[:, :, select_slices, :]
    label_vol = label_vol[:, :, select_slices]
