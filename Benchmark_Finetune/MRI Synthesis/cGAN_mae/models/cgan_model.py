...
class cGAN(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG_B = networks.define_G(
            opt.output_nc, opt.input_nc, opt.ngf, opt.norm,
            not opt.no_dropout, opt.init_type, self.gpu_ids
        )


        # If training and MAE checkpoint is provided, load and optionally freeze encoder
        if opt.mae_checkpoint_path:
            print(f"Loading pretrained MAE weights from {opt.mae_checkpoint_path}")
            mae_weights = torch.load(opt.mae_checkpoint_path, map_location='cpu')
            for netG in [self.netG_A, self.netG_B]:
                mae_state_dict = netG.model[0].state_dict()
                filtered_weights = {
                    k: v for k, v in mae_weights.items()
                    if k in mae_state_dict and v.size() == mae_state_dict[k].size()
                }
                netG.model[0].load_state_dict(filtered_weights, strict=False)

            if opt.mae_freeze:
                for param in self.netG_A.model[0].parameters():
                    param.requires_grad = False
                for param in self.netG_B.model[0].parameters():
                    param.requires_grad = False

        # Define discriminators and loss functions
        if self.isTrain:
            use_sigmoid = opt.no_lsgan
            self.netD_A = networks.define_D(
                opt.output_nc, opt.ndf, opt.n_layers_D,
                opt.norm, use_sigmoid, opt.init_type, self.gpu_ids
            )
            self.netD_B = networks.define_D(
                opt.input_nc, opt.ndf, opt.n_layers_D,
                opt.norm, use_sigmoid, opt.init_type, self.gpu_ids
            )
            ...
            self.schedulers = [networks.get_scheduler(optm, opt) for optm in self.optimizers]
        
        # Load full model checkpoints for testing or continued training
    ...
    def backward_G(self):
        ...
        adv_weight = self.opt.adv_weight
        # GAN loss D_A(G_A(A))
        fake_B = self.netG_A(self.real_A)
        ...
        loss_G = adv_weight * (loss_G_A + loss_G_B) + loss_cycle_A + loss_cycle_B
        loss_G.backward()
    ...
    def get_current_visuals(self):
        ...
        return ret_visuals, ret_tensors
