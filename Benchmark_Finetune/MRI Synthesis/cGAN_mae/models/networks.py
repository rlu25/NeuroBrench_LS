...
def define_G(input_nc, output_nc, ngf,  norm='batch', use_dropout=False, init_type='normal', gpu_ids=[], mae=False):
    ...
    if mae:
        netG = ResnetGeneratorMAE(input_nc, output_nc, ngf, norm_layer=norm_layer,
...
class ResnetGeneratorMAE(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=128, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[], padding_type='reflect'):
        assert (n_blocks >= 0)
        super(ResnetGeneratorMAE, self).__init__()
