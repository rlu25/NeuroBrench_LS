...
class BaseOptions():
    ...
    def initialize(self):    
        ...
        self.parser.add_argument('--name', type=str, default='Nips', help='name of the experiment. It decides where to store samples and models')
        self.parser.add_argument('--gpu_ids', type=str, default='2', help='gpu ids: e.g. 0  0,1,2, 0,2. use -1 for CPU')
        self.parser.add_argument('--checkpoints_dir', type=str, default='./ckpt', help='models are saved here')
        self.parser.add_argument('--model', type=str, default='local3D_trans', help='which model to use')
        ...
        self.parser.add_argument('--no_flip', default=False, type=bool, help='# threads for loading data')
    ...
    def parse(self, save=True):
        ...
        self.opt.device = torch.device(f'cuda:{self.opt.gpu_ids[0]}' if self.opt.gpu_ids else 'cpu')
        args = vars(self.opt)
