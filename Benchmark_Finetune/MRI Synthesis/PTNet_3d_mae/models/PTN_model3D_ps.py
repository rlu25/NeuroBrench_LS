...
from .transformer_block import Block, get_sinusoid_encoding, SWT4_3D
from torch import nn
import torch
import torch.nn.functional as F
from .transformer_block import SWT_up4_3D_ps as SWT_up4_3D
...
class PTNet(nn.Module):
    ...
    def __init__(self, img_size=[64, 64, 64], trans_type='performer', down_ratio=[1, 1, 2, 4],
        ...
        if not skip_connection:
            ...
            self.up_blocks = (nn.ModuleList(
                [SWT_up4_3D(in_channel= int(channels[-(i + 1)] +
                                            channels[-(i + 1)]/int(down_ratio[-(i + 1)] / down_ratio[-(i + 1) - 1])**3),
                            out_channel=channels[-(i + 1) - 1], patch=patch[-(i + 1)],
                            up_scale=int(down_ratio[-(i + 1)] / down_ratio[-(i + 1) - 1]),
                            padding=int((patch[-(i + 1)] - 1) / 2)) for i in range(1, len(down_ratio) - 2)]))
            self.up_blocks.append(SWT_up4_3D(in_channel=int(channels[1] + channels[1] / int(down_ratio[2] / down_ratio[1])**3),
                                             out_channel=channels[1], patch=patch[0],
                                             up_scale=int(down_ratio[2] / down_ratio[1]),
                                             padding=int((patch[0] - 1) / 2)))
            self.up_blocks.append(SWT_up4_3D(in_channel=int(channels[1] / int(down_ratio[1] / down_ratio[0])**3) + 1,
                                             out_channel=channels[1], patch=patch[0],
                                             up_scale=int(down_ratio[1] / down_ratio[0]),
                                             padding=int((patch[0] - 1) / 2)))
...
class PTNet_local(nn.Module):
    def __init__(self, img_size=[64, 64, 64], trans_type='performer', down_ratio=[1, 1, 2, 4, 8],
                 ...
                 patch=[3, 3, 3, 3, 3], embed_dim=128, depth=9, num_heads=4, mlp_ratio=2., drop_rate=0.,
                 qkv_bias=False, qk_scale=None, attn_drop_rate=0., drop_path_rate=0, norm_layer=nn.LayerNorm,
        ...
        self.GlobalGenerator = PTNet(img_size=[int(img_size[0] / 2), int(img_size[1] / 2), int(img_size[2] / 2)],
                                     ...
                                     channels=[1, 8, 16, 32, 64],
                                     patch=[3, 3, 3, 3, 3], embed_dim=128, depth=9, individual_use=False,
                                     skip_connection=True)
        self.down_blocks = nn.ModuleList(
            [SWT4_3D(in_channel=channels[i],
        ...
        if not skip_connection:
            ...
            self.up_blocks = (nn.ModuleList(
                [SWT_up4_3D(in_channel=int(channels[-(i + 1)]/int(down_ratio[-(i + 1)] / down_ratio[-(i + 1) - 1])**3+channels[-(i+1)]),
                            out_channel=channels[-(i + 1) - 1], patch=patch[-(i + 1)],
                            up_scale=int(down_ratio[-(i + 1)] / down_ratio[-(i + 1) - 1]),
                            padding=int((patch[-(i + 1)] - 1) / 2)) for i in range(1, len(down_ratio) - 2)]))
            self.up_blocks.append(SWT_up4_3D(in_channel=int(channels[1] +channels[1]/int(down_ratio[2] / down_ratio[1])**3 + 1),
                                             out_channel=channels[1], patch=patch[0],
                                             up_scale=int(down_ratio[2] / down_ratio[1]),
                                             padding=int((patch[0] - 1) / 2)))
            self.up_blocks.append(SWT_up4_3D(in_channel=int(channels[1] /int(down_ratio[1] / down_ratio[0])**3 + 1),
    ...
    def forward(self, x):
        ...
        if not self.sc:
            ...
            for up in self.up_blocks[-2:-1]:
                x = up(torch.cat((x, Global_res), dim=1), SC=SC[0], reshape=True, size=[x.shape[2], x.shape[3], x.shape[4]])
...
@register_model
def PTN(img_size=[48, 48, 48], **kwargs):
    model = PTNet(img_size=img_size, down_ratio=[1, 1, 2, 4, 8],
                  channels=[1, 32, 64, 128, 256],
                  patch=[7, 3, 3, 3], embed_dim=256, depth=9, individual_use=True)

    return model


@register_model
def PTN_local(img_size=[32, 32, 32], **kwargs):
