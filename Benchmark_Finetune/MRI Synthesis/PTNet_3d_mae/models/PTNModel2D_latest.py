    ...
    def __init__(self, embed_dim=64, depth=9, num_heads=2, mlp_ratio=2., drop_rate=0.,
        ...
        self.bottleneck = nn.ModuleList([
            Block(
                ...
                act_layer=nn.LeakyReLU(negative_slope=0.2))
            for i in range(depth)])
        self.sw = nn.Unfold(kernel_size=3, stride=1, padding=1)
        self.PE = nn.Parameter(data=get_sinusoid_encoding(n_position=int(224*256/4/4), d_hid=embed_dim), requires_grad=False)

        # self.final_proj = nn.Linear(32, 1)
        # self.final_LN = nn.LayerNorm(channels[1])

        self.bot_proj = nn.Linear(3**2, embed_dim)
        # self.bot_LN = nn.LayerNorm(channels[-1])
...
class PTNet_local_trans(nn.Module):
    def __init__(self, img_size=[224,256], trans_type='performer', down_ratio=[1, 1, 2, 4, 8],
        ...
        self.down_blocks = nn.ModuleList(
            [SWT4(in_channel=1, out_channel=16, patch=3, stride=1, padding=1), #64
             SWT4(in_channel=16, out_channel=32, patch=3, stride=2, padding=1), #32
             SWT4(in_channel=32, out_channel=64, patch=3, stride=2, padding=1), #16
             SWT4(in_channel=96, out_channel=192, patch=3, stride=2, padding=1)]) #8
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]
        self.bottleneck = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
    ...
    def forward(self, x):
        x0 = x
        Global_feat = self.GlobalGenerator(F.interpolate(x, scale_factor=0.25, mode='bilinear', align_corners=True))
        if not self.sc:
            for i, down in enumerate(self.down_blocks[:-1]):
                x = down(x)
                B, HW, C = x.shape
...
@register_model
def PTN_Local_Trans(img_size=[224, 256], **kwargs):
    model = PTNet_local_trans()
