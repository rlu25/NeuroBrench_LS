    ...
    def save_visualization(self, epoch):
        ...
        for k, v in self.visual_dict.items():
            if 'mask' in k:
                ...
                vis[k] = util.tensor2label(v[0, slc_num, :, :].unsqueeze(0), 2)  # No `D` index

            else:
                # vis[k] = util.tensor2im(v[0, :, :, :, slc_num])
                if v.shape[1]==1:
                    vis[k] = util.tensor2im(v[0, 0, :, :].unsqueeze(0))  # No `D` index
                else:
                    vis[k] = util.tensor2im(v[0, slc_num, :, :].unsqueeze(0))  # No `D` index
